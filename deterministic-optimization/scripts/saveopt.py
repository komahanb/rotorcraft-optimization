...
parser.add_argument('--smoothness', type=str , default='no', help='yes/no')
...
class RotorAssemblyOpt:
    def __init__(self, comm,
        ...
        if self.smoothness is True:
            self.ncon = len(self.funcs) - 1 + 2*(num_vars-4)
            if self.ncon <= 0:
    ...
    def evalObjCon(self, x):
        ...
        # 1 - sigma/sigma_max >= 0
        con[0] = 5.0*self.funcVals[1] - 1.0
        print ("stress constraint : ", con[0])
    ...
    def evalSmoothness(self, x):
        ...
        con = np.zeros(2*(self.nvars-4), TACS.dtype)
        j = 0 # tracks variable
        k = 0 # tracks constraint
        for i in range(self.nvars-4):
    ...
    def evalSmoothnessGrad(self,x):
        grad = np.zeros([2*(self.nvars-4),self.nvars], TACS.dtype)
        j = 0
        k = 0
        for i in range(self.nvars-4):
            if j == 47 or j == 95 or j == 143 or j == 191:
...
optproblem = RotorAssemblyOpt(MPI.COMM_WORLD, problem, num_vars, smoothness)
...
if optproblem.comm.Get_rank() ==0:
    print("optimization solution : ", opt_prob.solution(0))
