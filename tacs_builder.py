...
class TACSBody:
    ...
    def __init__(self,
        ...
        assert(self.nelems == len(self.ptr)-1)

        t = {}
        for n in self.conn:
            t[n] = n

        self.nodes = np.array(list(t.keys()), dtype=np.intc)

        # Determine the type of boundary for elements associated with
        # this body
        self.processBoundaryNodes()

        self.nnodes = int(len(self.xpts)/3)
        ...
        if self.bcs is None or len(self.bcs) == 0:
            ...
            for eidx in range(self.nelems):
                ...
                if ncommon == 0:
                    ...
                    self.bctype.append(TACSBCType.NA)
                    # print "corner node skipped", ncommon, econn, common
                    # raise
    ...
    def setLoads(self,struct_loads):
        ...
        """
        assert(len(struct_loads) == 8*len(self.xpts)/3)
...
class TACSBuilder:
    ...
    def createTACS(self, comm, xpts, conn, ptr, elems, bcs,
                   ...
                   print_details=2, ndof=8, bcptr=None, bcvars=None):
        """
    ...
    def createTACSFromBDF(comm,
        ...
        mesh.setConvertToCoordinate(convert_mesh) # not available in new tacs
    ...
    def addMITCShellBody(self,
        ...
        mesh.setConvertToCoordinate(bdf_type_flag) ## fix not available in new tacs
        mesh.scanBDFFile(mesh_dir+bdffile)
    ...
    def addMITC9ShellBody(self,
        ...
        # Load mesh from BDF file
        mesh = TACS.MeshLoader(self.comm)
        mesh.setConvertToCoordinate(gmsh) ## fix not available in new tacs
        mesh.scanBDFFile(mesh_dir+bdffile)
    ...
    ##         # Offset the pointer
    ##         self.setNodePointer(solid_elem.getNumNodes())
    ...
    def addBeamBody(self, name, cbar, vInit, wInit, gravity, beamStiff):
        """
        ...
        # Create the local node numbers
        nodes = []
        num_nodes = int(len(xpts)/3)
        ...
        if (isinstance(beamStiff, constitutive.TimoshenkoConstitutive)):
    ...
    def addRigidLink(self, rigidCompA, bodyB, type=TACSBCType.FACE):
        ...
        for bc in bcs:
            ...
            xpts.append(bodyB.xpts[0])
            xpts.append(bodyB.xpts[1])
            xpts.append(bodyB.xpts[2])
    ...
    def addFlexLink(self,
        ...
        if type == TACSBCType.EDGE:
            ...
            raise"unsupported connection type for flexible lnk"
    ...
    def addRevoluteConstraint(self, ref_point, revaxis, bodyA,
        ...
        ##     else:
        ##         revcon = self.ehelper.createRevoluteConstraint(ref_point, revaxis,
        ##                                                        fixed_ref_point,
        ##                                                        inertial_rev_axis,
        ##                                                        None,
        ##                                                        None)
        ...
        # Dummy link locations
        xpts.append(ref_point[0])
        xpts.append(ref_point[1])
        xpts.append(ref_point[2])
        ...
        conn.append(bodyA.conn[-1]) # last node of body A
        if bodyB is not None:
            conn.append(bodyB.conn[0]) # first node of body B
    ...
    def addRevoluteDriver(self, revaxis, speed, drivenComp):
        ...
        for bc in [drivenComp.bcs[0]]: # plate has multiple nodes fixed at one end, but beam has only one ## fix
            # Append the element to list
            elems.append(driver)
    ...
    def rigidBody(self, filename):
        ...
        assert(self.isPositiveDefinite(Jref)==1) # (sanity check)

        # print mass*np.matmul(self.skew(xcg),self.skew(xcg))
    ...
    def body(self, filename, type):
        ...
        if type == TACSBodyType.RIGID:
            ...
            raise "unsupported body type to create"
...
class ElementHelper:
    ...
    def createBeamElement(self, vInit, wInit, gravity, beamStiff):
        assert(isinstance(beamStiff, constitutive.TimoshenkoConstitutive)==1)
        v = elements.GibbsVector(vInit[0], vInit[1], vInit[2])
        w = elements.GibbsVector(wInit[0], wInit[1], wInit[2])
        g = elements.GibbsVector(gravity[0], gravity[1], gravity[2])
...
class TACSDynamicsProblem(TACSProblem):
    ...
    def getArguments(self):
        ...
        parser.add_argument('--integration_order'     , type=int    , default=1          , help='The order of accuracy of the integration scheme')
    ...
    def initialize(self):
        ...
        #print "reordered nodes", self.newnodes
        #self.oldnodes = list(np.arange(len(self.newnodes)))
        #print "indices", self.newnodes.index(self.oldnodes)

        #old node i -> newNodeIndices[i]
        #odevec = self.tacs.createNodeVec()
        #elf.tacs.getNodes(nodevec)

        # Update the body list with new node numbers
        #nodearray = nodevec.getArray()
        #print "node array" , len(nodearray)/3, len(self.newnodes)
        #stop
    ...
    def getForces(self, tindex=None):
        ...
        for body in self.tacs_body_list:
            ...
            if body.btype == TACSBodyType.FLEXIBLE:
                ...
                # Store the force values if provided
                if body.forcing is not None:
                    body.forcing(tindex, body)
                else:
                    body.forces[:] = 0.0

                # Check this logic (are we messing up nodes??)
